...
main()
{
	...
	Line * lp = NULL, *p = NULL;
	char filePath[MAXLINELENGTH], fileName[MAXLINELENGTH];
loop:;// Let user to input the file path.
	...
	filePath[k] = '\0';
	printf("Press y to conform the file path:'%s' or any other key to rewrite it:", filePath);
	if (getchar() == 'y')
	{
		getchar();
		printf("The file path:'%s' has been conformed.\n", filePath);
	}
	else {
		getchar();
		goto loop;
	}
	while (filePath[k] != '\\' && filePath[k] != '/' && k > 0)
	{
		k--;
	}
	k++;
	for (t = 0; k < strlen(filePath) + 1; k++)
	{
		fileName[t++] = filePath[k];
	}
	...
	p = lp;
	printf("Please comform the content of the file '%s'.\n", fileName);
